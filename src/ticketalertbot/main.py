...
from config.config import FILE_PATH, SUBSCRIBERS_FILE, REQUESTS_FILE, URL, BUTTON_XPATH, LOG_FILE
...
from utils.telegram_utils import fetch_updates, process_updates, notify_subscribers, \
    notify_info_requesters, notify_based_on_status
...
async def main():
    ...
    try:
        ...
        try:
            ...
            # Общая функция для проверки билетов
            async def check_and_handle():
                ...
                await notify_based_on_status(tickets_available, new_date, subscribers, info_requests, BOT_TOKEN)
                return tickets_available

            if not status:
                await check_and_handle()
                return
